...
<style>
    ...
    .metric-value { font-size: 2.2rem; font-weight: 700; color: #3b5bdb; margin: 0; }
    .metric-label { font-size: 0.85rem; color: #6b7a99; margin-top: 4px; letter-spacing: 0.05em; text-transform: uppercase; }
    .section-title {
        font-size: 1.1rem; font-weight: 600; color: #2d3a5e;
        border-left: 3px solid #3b5bdb; padding-left: 10px;
        margin-bottom: 16px;
    }
    div[data-testid="stTabs"] button { color: #6b7a99; }
    div[data-testid="stTabs"] button[aria-selected="true"] { color: #3b5bdb; border-bottom-color: #3b5bdb; }
</style>
...
def light_layout(**kwargs):
    base = dict(plot_bgcolor=BG, paper_bgcolor=BG, font_color=FONT_COLOR)
    base.update(kwargs)
    return base
...
with tab1:
    ...
    with col_l:
        ...
        fig_bar.update_traces(textposition="outside", textfont_size=11, textfont_color=FONT_COLOR)
        fig_bar.update_layout(**light_layout(
            yaxis_title="", xaxis_title="Citations",
            coloraxis_showscale=False,
            margin=dict(l=0, r=40, t=10, b=10), height=420,
        ))
    ...
    with col_r:
        if "country" in institution_df.columns:
            ...
            fig_pie.update_layout(**light_layout(
                showlegend=False, margin=dict(l=0, r=0, t=10, b=10), height=420,
            ))
    ...
    if "country" in inst_filtered.columns:
        ...
        fig_tree = px.treemap(
            inst_top, path=["country", "name"], values="count",
            color="count", color_continuous_scale="Blues",
        )
        fig_tree.update_layout(**light_layout(margin=dict(l=0, r=0, t=10, b=10), height=350))
...
with tab2:
    ...
    with col_l:
        ...
        fig_pie2.update_layout(**light_layout(
            showlegend=False, margin=dict(l=0, r=0, t=10, b=10), height=400,
        ))
    ...
    with col_r:
        ...
        fig_topic = px.bar(
            topic_sorted, x="count", y="name", orientation="h",
            color="count", color_continuous_scale="Teal", text="count",
        )
        fig_topic.update_traces(textposition="outside", textfont_size=11, textfont_color=FONT_COLOR)
        fig_topic.update_layout(**light_layout(
            yaxis_title="", xaxis_title="Citations",
            coloraxis_showscale=False,
            margin=dict(l=0, r=40, t=10, b=10), height=400,
        ))
        fig_topic.update_xaxes(gridcolor=GRID_COLOR)
...
with tab3:
    ...
    with col_l:
        ...
        fig_donut.update_layout(**light_layout(
    ...
    with col_r:
        ...
        fig_hbar = go.Figure(go.Bar(
            x=type_sorted["count"], y=type_sorted["name"], orientation="h",
            marker=dict(color=type_sorted["count"], colorscale="Purples",
                        line=dict(color="rgba(0,0,0,0)")),
            text=type_sorted["count"], textposition="outside",
        ))
        fig_hbar.update_layout(**light_layout(
            xaxis=dict(title="Citations", gridcolor=GRID_COLOR),
            yaxis_title="", margin=dict(l=0, r=60, t=10, b=10), height=420,
        ))
    ...
    fig_cum.add_trace(go.Bar(
        ...
        name="Count", marker_color="#4c72b0", opacity=0.75,
    ))
    fig_cum.add_trace(go.Scatter(
        x=type_cum["name"], y=type_cum["cumulative_pct"],
        name="Cumulative %", yaxis="y2",
        line=dict(color="#e05c5c", width=2.5), marker=dict(size=6),
    ))
    fig_cum.update_layout(**light_layout(
        yaxis=dict(title="Count", gridcolor=GRID_COLOR),
        yaxis2=dict(title="Cumulative %", overlaying="y", side="right", range=[0, 105]),
        legend=dict(orientation="h", y=1.1),
        margin=dict(l=0, r=60, t=30, b=10), height=320,
    ))
...
# ── Footer ────────────────────────────────────────────────
st.markdown("---")
st.markdown('<p style="text-align:center; color:#9aa5c0; font-size:0.8rem;">Citation Source Analytics · Built with Streamlit & Plotly</p>', unsafe_allow_html=True)
